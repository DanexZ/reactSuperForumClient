...
const UserProfile = () => {
    ...
    const [{ userName, password, passwordConfirm, resultMsg, isSubmitDisabled }, dispatch] = useReducer(userReducer, {
        userName: "",
        password: "*********",
        passwordConfirm: "*********",
        resultMsg: "",
        ...
        if (user) {
            ...
            getUserThreads(user.id).then((items) => {
                ...
                const threadList = items.map((th: Thread) => {
                    ...
                    for (let i = 0; i < th.threadItems.length; i++) threadItemsInThreadList.push(th.threadItems[i]);
                ...
                const threadItemList = threadItemsInThreadList.map((ti: ThreadItem) => (
                    <li key={`user-th-${ti.thread_id}`}>
                        <Link to={`/thread/${ti.thread_id}`} className="userprofile-link">
                        {ti.body}
                        </Link>
                    </li>
                ));
            ...
            <form className="userprofile-content-container">
                ...
                <div className="userprofile-password">
                    <div>
                        ...
                        />
                        <button className="action-btn" disabled={isSubmitDisabled}>Zmień hasło</button>
                    </div>
