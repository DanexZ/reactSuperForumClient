...
export const useRefreshReduxMe = (): UseRefreshReduxMeResult => {
    const [execMe, { data }] = useLazyQuery(Me);
    const reduxDispatcher = useDispatch();

    const deleteMe = () => reduxDispatcher({type: UserProfileSetType, payload: null});

    const updateMe = () => {
        if (data && data.me && data.me.userName) reduxDispatcher({type: UserProfileSetType, payload: data.me});
    };
